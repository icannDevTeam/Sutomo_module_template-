 ...
 *
 * KPIs:
 *   - Pending Agreement: contract uploaded by Yayasan but teacher hasn't e-signed yet.
...
class ContractContinuation extends Page
{
    ...
    public function getStageFor(LetterOfIntent $loi): string
    {
        ...
        if (! is_null($loi->yayasan_contract_uploaded_at)) {
            return 'pending_agreement'; // contract is here, teacher needs to e-sign
        }
    ...
    public function recordBukuInduk(int $loiId): void
    {
        ...
        if (is_null($loi->agreement_signed_at)) {
            Notification::make()->title('Teacher must e-sign the agreement first')->danger()->send();
            return;
        }
        $loi->update([
